...
export default function ResponsiveMinimalisticGitHubDashboard({ initialUsername }: { initialUsername?: string }) {
  ...
  const shareAsPNG = async () => {
    ...
    loadingDiv.innerHTML = `
      <div class="bg-black/40 backdrop-blur-xl rounded-xl border border-white/10 p-4 flex items-center gap-3">
        ...
        <span class="text-green-400">Generating image...</span>
      </div>
    `;
    ...
    try {
      ...
      const shareUrls = {
        ...
        embed: `[![My ChillGits Score](${shareableUrl}/og)](${shareableUrl})`
      };
      ...
      menu.className = `
        ...
        p-4 z-50 min-w-[300px] max-w-[90vw] shadow-xl
      `;
      ...
      const menuContent = `
        <div class="space-y-4">
          <div class="space-y-1">
            ...
            <button id="copy-embed" class="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 transition-colors text-white/70 hover:text-white/90">
              <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M16 18l2-2v-3"></path>
                <path d="M8 6l-2 2v3"></path>
                <path d="M12 12m-2 0a2 2 0 1 0 4 0a2 2 0 1 0-4 0"></path>
                <path d="M3 12a9 9 0 1 0 18 0a9 9 0 1 0-18 0"></path>
              </svg>
              <span class="text-sm">Copy Embed Code</span>
            </button>
          </div>
          <div class="mt-2 p-2 bg-black/30 rounded-lg">
            <code class="text-xs text-green-300/70 break-all">${shareUrls.embed}</code>
          </div>
        </div>
      `;
      ...
      const copyEmbedButton = menu.querySelector('#copy-embed');
      copyEmbedButton?.addEventListener('click', async () => {
        try {
          await navigator.clipboard.writeText(shareUrls.embed);
          const originalContent = copyEmbedButton.innerHTML;
          copyEmbedButton.innerHTML = `
            <svg class="w-4 h-4 text-green-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <polyline points="20 6 9 17 4 12"></polyline>
            </svg>
            <span class="text-sm text-green-400">Copied!</span>
          `;
          setTimeout(() => {
            copyEmbedButton.innerHTML = originalContent;
          }, 2000);
        } catch (error) {
